...
export default function UserQuickEditForm({ currentUser, open, onClose }: Props) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      regionIds: currentUser.UserRegion.map((region) => region.Region.id),
      territoryIds: currentUser.UserTerritory.map((territory) => territory.Territory.id),
    }),
    [currentUser]
  );
          ...
          <Grid container spacing={2} rowSpacing={4} sx={{ mt: 1 }}>
            ...
            <Grid item xs={6}>
              <RHFAutocompleteAsyncOnSearch
                initialValues={currentUser.UserRegion.map((region) => ({
                  id: region.Region.id,
